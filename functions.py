...
def cv_draw_landmark(img_ori, pts, box=None, color=GREEN, size=1): 
    ...
    for key, val in facial_landmarks_idxs.items():
        points = np.array(np.transpose(pts)[val[0]:val[1]], np.int32)[:, 0:2]
        fill_poly = bool(key in ("right_eye", "left_eye", "mouth"))
        if key == 'jaw':
            color = RED
        else:
            color = GREEN
        cv2.polylines(img, [points], fill_poly, color, 1)
    ...
    if n <= 106:
        ...
        for i in range(0, n, sep):
            cv2.circle(img, (int(round(pts[0, i])), int(round(pts[1, i]))), size, color, 2)
...
def cv_draw_landmark_pca(img_ori, pts, box=None, color=GREEN, size=1): 
    # pts: (3, 68)
    img = img_ori.copy()
