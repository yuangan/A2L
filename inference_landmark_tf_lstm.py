...
from models import Generator, MappingNetwork, StyleEncoder, LmkEncoder, MotDecoder, DiscriminatorMot
...
def build_model(model_params):
    ...
    discriminator = DiscriminatorMot(args.num_domains)
...
model_mot_path = '/home/gy/gy/benchmark/A2L_StarG2/Models/A2L_mot_M30_lstm2/epoch_01996.pth'
wav_path = '/data3/MEAD/M003/wav/surprised/level_3/001.wav'
...
extend_lmks = torch.zeros((1, 96, 68, 2)).float().to('cuda')
mel_size = len_test
extend_lmks[0, :mel_size, :, :] = input_lmks[0, :mel_size,:,:]
print('extend_lmks: ', extend_lmks.shape)
...
#         print('infer 134 f0: ', f0_feat.shape) #torch.Size([1, 256, 10, 299])
        ...
        # assert(0)
        lm_feature = models_mot.lmk_encoder(extend_lmks)
        motion_fake = models_mot.mot_decoder(lm_feature, audio_feature, length_mots)
        ...
        writer = imageio.get_writer('./lmk_result/motion_fake.mp4', fps=30)
        fake_fl = visual_landmarks(example_landmark, motion_fake[:,:length,:,:], writer)
        writer = imageio.get_writer('./lmk_result/motion_gt.mp4', fps=30)
        visual_landmarks(example_landmark, motion_gt, writer)


        np.save(f'./lmk_result/{source_emo}_{bname[:-4]}_to_{key}_fake', fake_fl)
